...
%   checks failed with actionable next steps.
    ...
    % `exist(name, 'file')` returns 3 for MEX files, 2 for .m files.
    % Use 3 to verify the compiled MEX is actually present — not just
    % the .m wrapper (which would exist even on a freshly cloned tree
    % where MEX hasn't been built yet).
    ...
    results(end+1) = ok('MEX: moglcore available (MOGL)', ...
        exist('moglcore', 'file') == 3, ...
        'Build lib/MOGL: cd there, run make() (no arguments).');
