classdef Movie < stage.core.Stimulus
    ...
    methods
        ...
        function init(obj, canvas)
            ...
            % Use the cross-platform ffmpeg-subprocess backend
            % (TASK-006). Replaces AVbin, which is Windows-only and
            % has a stale-frame bug in nextImage (see
            % spec/TASKS.md § TASK-006). To roll back to AVbin
            % for troubleshooting, change this line to
            % `source = VideoSource(obj.filename);` and restart
            % Stage.
